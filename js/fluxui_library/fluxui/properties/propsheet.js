...
( function( $ ) {
	...
	var clazz = $.fn.fluxui.$class.create( {
		...
		methods : {
			...
			bindEvents : function() {
				var me = this;
				this.addListener( 'properties.' + this.ens + '.*', function( $ns, $evt ) {
					me.handlePropertyChange( $($evt.target) );
				} );
				this.addListener( 'events.' + this.ens + '.changed', function( $ns, $cls ) {
			...
			// Standard set of property fields to be updated. This function can be overwritten by specific propsheets.
			updatePropertyFields : function( $cls, $prefix ) {
				var d = eventDispatcher;
				d.dispatch( this, $prefix + '.width.changed', Math.round( parseFloat( $cls.width() ) ) );
				d.dispatch( this, $prefix + '.height.changed', Math.round( parseFloat( $cls.height() ) ) );
				d.dispatch( this, $prefix + '.left.changed', Math.round( parseFloat( $cls.x() ) ) );
				d.dispatch( this, $prefix + '.top.changed', Math.round( parseFloat( $cls.y() ) ) );
				...
				if ( !!$cls.text ) d.dispatch( this, $prefix + '.text.changed', $cls.text() );
			},
			...
			handlePropertyChange : function( $control ) {
				...
				switch( fid ) {
					case 'flux-id':
						t.fluxid( s );
						d.dispatch( this, 'events.explorer.update' );
						break;
					case 'left':
						i.x( n );
						t.x( n, 1 );
