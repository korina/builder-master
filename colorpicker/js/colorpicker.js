...
(function ($) {
	var ColorPicker = function () {
		var
			...
			HexToRGBA = function (hex) {
				...
				var hexa = parseInt( ( hex ).substr( 0, 4 ), 16 );
				var hexb = parseInt( ( hex ).substr( 4 ), 16 );
				var rgba = { r: ( hexa & 0xFF00 ) >> 8, g: hexa & 0xFF, b: ( hexb & 0xFF00 ) >> 8, a: hexb & 0xFF };
				return rgba;
			},
		...
		return {
			...
			setColor: function(col) {
				if (typeof col == 'string') {
					...
					col = fixHSBA(col);
				} else if (!!col.colors && col.colors.length > 0) {
					var alpha = ( !isNaN(arguments[1]) ) ? base16( arguments[1] * 255 ) : 'ff';
					col = col.colors[0].rgb + alpha;
					col = HexToRGBA( col );
					col = RGBAToHSBA( col );
				} else {
					return this;
	...
	$.fn.extend({
		...
		ColorPickerSetAlpha: ColorPicker.setAlpha
	});
})(jQuery)